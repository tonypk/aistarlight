...
export const useTransactionStore = defineStore("transaction", () => {
  ...
  async function updateTransaction(
    ...
    const res = await reconciliationApi.updateTransaction(
      sessionId,
      txnId,
      data,
    );
  ...
  async function fetchAnomalies(
    ...
    const res = await reconciliationApi.listAnomalies(
      sessionId,
      page,
      50,
      statusFilter,
  ...
  async function bulkUpdateTransactions(
    ...
    loading.value = true;
    try {
      const res = await reconciliationApi.bulkUpdateTransactions(
        sessionId,
        items,
      );
      await fetchTransactions(sessionId);
  ...
  async function exportExcel(sessionId: string) {
    let url = "";
    try {
      const res = await reconciliationApi.exportExcel(sessionId);
      url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute(
        "download",
        `transactions_${currentSession.value?.period ?? sessionId}.xlsx`,
      );
  ...
  async function exportCsv(sessionId: string) {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `reconciliation_${currentSession.value?.period ?? sessionId}.csv`,
      );
      document.body.appendChild(link);
      link.click();
      link.remove();
