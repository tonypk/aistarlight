...
import type { SupplierCreateData, SupplierUpdateData } from '../types/withholding'
...
export const withholdingApi = {
  ...
  deleteSupplier: (id: string) =>
    client.delete(`/withholding/suppliers/${id}`),
  ...
  // Certificates
  listCertificates: (page = 1, limit = 50, period?: string, supplierId?: string) =>
  ...
  downloadCertificate: (certId: string) =>
    client.get(`/withholding/certificates/${certId}/download`, { responseType: 'blob' }),
  ...
  // EWT Rates Reference
  listEwtRates: () =>
    client.get('/withholding/ewt-rates'),
}
