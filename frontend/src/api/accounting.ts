...
export const accountsApi = {
  ...
  get: (id: string) => client.get(`/accounts/${id}`),
  create: (data: {
  ...
  update: (
    id: string,
    data: { name?: string; description?: string; sub_type?: string },
  ) => client.put(`/accounts/${id}`, data),
  delete: (id: string) => client.delete(`/accounts/${id}`),
  seed: () => client.post("/accounts/seed"),
...
export const journalApi = {
  ...
  get: (id: string) => client.get(`/journal-entries/${id}`),
  create: (data: {
    entry_date: string;
    reference?: string;
    description?: string;
  ...
  post: (id: string) => client.post(`/journal-entries/${id}/post`),
  reverse: (id: string) => client.post(`/journal-entries/${id}/reverse`),
};
...
export const receiptBridgeApi = {
  convert: (batchId: string, sessionId: string) =>
    client.post(`/receipts/batches/${batchId}/convert`, {
...
export const taxBridgeApi = {
  ...
  getLatestDraft: (formType: string) =>
    client.get("/tax/drafts/latest", { params: { form_type: formType } }),
};
...
export const periodsApi = {
  list: () => client.get("/accounting-periods"),
  ...
  close: (id: string) => client.post(`/accounting-periods/${id}/close`),
  reopen: (id: string) => client.post(`/accounting-periods/${id}/reopen`),
};
