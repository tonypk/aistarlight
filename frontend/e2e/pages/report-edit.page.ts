...
export class ReportEditPage {
  ...
  getField(fieldId: string): Locator {
    return this.page
      .locator(`[data-testid="edit-field-${fieldId}"]`)
      .or(
        this.page.locator(
          `.field-row:has(.label:has-text("${fieldId}")) .field-input`,
        ),
      );
  }

  getFieldByLine(lineNumber: string): Locator {
    // Use :text-is() for exact match (avoids "1" matching "10", "11", etc.)
    return this.page.locator(
      `.field-row:has(.line-no:text-is("${lineNumber}")) .field-input`,
    );
  }
  ...
  async save() {
    ...
    await expect(this.saveButton).not.toContainText("Saving", {
      timeout: 10_000,
    });
  }
