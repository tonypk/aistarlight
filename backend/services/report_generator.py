...
def generate_pdf_report(report_type: str, data: dict[str, Any], tenant_info: dict) -> str:
    ...
    elif report_type == "BIR_2316":
        _generate_bir_2316_pdf(filepath, data, tenant_info)
    else:
...
def _generate_bir_2316_pdf(filepath: str, data: dict, tenant_info: dict) -> None:
    """Generate BIR 2316 Certificate of Compensation Payment/Tax Withheld PDF."""
    c = canvas.Canvas(filepath, pagesize=letter)
    width, height = letter
    ...
    c.setFillColor(white)
    c.setFont("Helvetica-Bold", 18)
    c.drawCentredString(width / 2, height - 30, "BIR Form No. 2316")
    c.setFont("Helvetica", 10)
    c.drawCentredString(width / 2, height - 48, "Certificate of Compensation Payment / Tax Withheld")
    c.setFont("Helvetica", 7)
    c.drawCentredString(width / 2, height - 60, "Republic of the Philippines - Department of Finance - Bureau of Internal Revenue")

    # Part I: Employee & Employer Info
    y = height - 90
    y = _draw_section_header(c, y, width, "Part I - Employee & Employer Information")
    c.setFont("Helvetica", 8)
    for label, value in [
        ("Employee Name", data.get("employee_name", "")),
        ("Employee TIN", data.get("employee_tin", "")),
        ("Employer / Company", data.get("employer_name", tenant_info.get("company_name", ""))),
        ("Employer TIN", data.get("employer_tin", tenant_info.get("tin_number", ""))),
        ("Taxable Year", data.get("period", "")),
    ]:
        y -= 14
        c.setFont("Helvetica", 8)
    ...
    y -= 20
    y = _draw_section_header(c, y, width, "Part II - Compensation from Present Employer")
    y = _draw_line_item(c, y, width, "1", "Gross Compensation",
                        data.get("present_employer_compensation", "0"))
    y = _draw_line_item(c, y, width, "2", "Less: Non-Taxable Compensation",
                        data.get("present_employer_nontaxable", "0"))
    y = _draw_line_item(c, y, width, "3", "Taxable Compensation (Line 1 - Line 2)",
                        data.get("present_employer_taxable", "0"), bold=True)

    # Part III: Previous Employer
    y -= 8
    y = _draw_section_header(c, y, width, "Part III - Compensation from Previous Employer(s)")
    y = _draw_line_item(c, y, width, "4", "Gross Compensation (Previous)",
                        data.get("previous_employer_compensation", "0"))
    y = _draw_line_item(c, y, width, "5", "Less: Non-Taxable (Previous)",
                        data.get("previous_employer_nontaxable", "0"))
    y = _draw_line_item(c, y, width, "6", "Taxable Compensation (Previous)",
                        data.get("previous_employer_taxable", "0"), bold=True)

    # Part IV: Totals
    y -= 8
    y = _draw_section_header(c, y, width, "Part IV - Total Compensation Summary")
    y = _draw_line_item(c, y, width, "7", "Total Compensation (Line 1 + Line 4)",
                        data.get("total_compensation", "0"))
    y = _draw_line_item(c, y, width, "8", "Total Non-Taxable Compensation (Line 2 + Line 5)",
                        data.get("total_nontaxable_compensation", "0"))
    y = _draw_line_item(c, y, width, "9", "Total Taxable Compensation (Line 3 + Line 6)",
                        data.get("total_taxable_compensation", "0"), bold=True)

    # Part V: Tax Computation
    y -= 8
    y = _draw_section_header(c, y, width, "Part V - Tax Computation & Year-End Adjustment")
    y = _draw_line_item(c, y, width, "10", "Tax Due (per Graduated Tax Table)",
                        data.get("tax_due", "0"), bold=True)
    y = _draw_line_item(c, y, width, "11", "Tax Withheld (Present Employer)",
                        data.get("tax_withheld_present", "0"))
    y = _draw_line_item(c, y, width, "12", "Tax Withheld (Previous Employer)",
                        data.get("tax_withheld_previous", "0"))
    y = _draw_line_item(c, y, width, "13", "Total Tax Withheld (Line 11 + Line 12)",
                        data.get("total_tax_withheld", "0"), bold=True)

    # Year-end adjustment - highlighted
    y -= 4
    amount_refunded = data.get("amount_refunded", "0")
    amount_still_due = data.get("amount_still_due", "0")
    is_refund = float(amount_refunded or 0) > 0

    c.setFillColor(HexColor("#f0fdf4") if is_refund else HexColor("#f0f4ff"))
    c.rect(54, y - 14, width - 108, 22, fill=1, stroke=0)
    accent = HexColor("#16a34a") if is_refund else _ACCENT
    ...
    c.setFont("Helvetica-Bold", 10)
    if is_refund:
        c.drawString(60, y - 8, "14")
        c.drawString(90, y - 8, "AMOUNT REFUNDED TO EMPLOYEE")
        c.setFillColor(HexColor("#16a34a"))
        c.drawRightString(width - 60, y - 8, f"PHP {_format_amount(amount_refunded)}")
    else:
        c.drawString(60, y - 8, "14")
        c.drawString(90, y - 8, "AMOUNT STILL DUE FROM EMPLOYEE")
        c.setFillColor(_ACCENT)
        c.drawRightString(width - 60, y - 8, f"PHP {_format_amount(amount_still_due)}")
    y -= 24
    ...
    c.setFont("Helvetica", 6)
    c.setFillColor(HexColor("#888888"))
    c.drawCentredString(width / 2, 30, "Generated by AIStarlight | Draft certificate for review purposes")
    c.save()
